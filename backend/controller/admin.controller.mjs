...
export class AdminController {
  ...
  async dashboardtoggle(req, res) {
    try {
      let refreshToken =
        TokenUtility.getToken(req, "refresh") ||
        TokenUtility.getToken(req, "access");
      ...
      let decoded = TokenUtility.verifyToken(refreshToken);

      const user = await signup.findOne({ email: decoded.email });

      let status = user.role === "admin" ? true : false;
  ...
  async removeUser(req, res) {
    try {
      ...
      await AC.deleteUser(userId);
      const backup = await AC.findUser(userId);
      if (!backup) console.log("Deleted backup:");
