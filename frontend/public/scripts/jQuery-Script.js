...
$(async function () {
  ...
  $(".logout-btn, #logout-button, #account-logout").on("click", function (e) {
    e.preventDefault();

    const $overlay = $("#loadingOverlay");
    const $message = $("#loadingOverlay p");
  ...
  $(".loginform").on("submit", function (e) {
    ...
    $.ajax({
      ...
      beforeSend: function () {
        $("#loadingOverlay p").text("Logging in...");
      },

      success: function (res) {
        $("#loadingOverlay").html(`
                       <div style="display:flex; flex-direction:column; align-items:center; gap:10px;">
                            <img src="https://media.giphy.com/media/v1.Y2lkPTc5MGI3NjExNGhub3RjOWRhdzdpMnp3cGYwZ2I4dWVyNnRkMTFwYTk4ajJmZDN2bCZlcD12MV9naWZzX3NlYXJjaCZjdD1n/eQYE2toD7Pnpu/giphy.gif"
                            style="width:400px; height:auto; border-radius:80px;"/>
                            <big><p>${res}</p></big>
                       </div>`);
        setTimeout(() => {
          $("#loadingOverlay").fadeOut();
